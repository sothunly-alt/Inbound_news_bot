...
Do NOT include a Source line — a button is added separately."""
    else:
        format_rules = """Return EXACTLY this structure (no extra sections, no preamble):
**<short title>**
▸ What happened: <one sentence>
▸ Why it matters: <one sentence>
▸ Context: <one short sentence with background or comparison>
Do NOT include a Source line — a button is added separately.
If sources disagree on a fact, say so clearly in Context instead of guessing."""
...
Return ONLY the formatted post text, nothing else."""
    ...
    return format_story_html(body, header=header)
